...
a.on("click",a=>{$("#story .attribute").removeClass("highlight");// Declare some variables we need.
let b,c,d,e,f,g,h,j,k,l,m=$("fieldset.original input[ type=\"radio\" ]:checked").val(),n=$("fieldset.original input[ type=\"checkbox\" ]:checked").val(),o=$("fieldset.gender input[ type=\"radio\" ]:checked").val(),p=$("fieldset.ethnicity input[ type=\"radio\" ]:checked").val();// Grab the user-selected attributes from the form.
null!=m&&null!=o&&null!=p&&(a.preventDefault(),$.getJSON("/cctp623/data/people.json",a=>{// If we have a person...
if($.each(a,(a,i)=>{o===i.gender&&p===i.ethnicity&&(b=i.name,c=i.title,d=i.child,e=i.sibling,f=i.pronounSubject,g=i.pronounObject,h=i.pronounPossAdj,j=i.pronounBeingPresent,k=i.pronounBeingPast,l="/cctp623/photos/"+i.photo+".jpg")}),b){// If "same story" is selected...
let a=n?n:m;// Load the correct story piece.
$("#story .text").load("/cctp623/stories/"+a+".html",()=>{$("#story ."+m).addClass("highlight"),$("#story .name."+m).html(b),$("#story .title."+m).html(c),$("#story .child."+m).html(d),$("#story .sibling."+m).html(e),$("#story .pronoun-subject."+m).html(f),$("#story .pronoun-object."+m).html(g),$("#story .pronoun-poss-adj."+m).html(h),$("#story .pronoun-being-present."+m).html(j),$("#story .pronoun-being-past."+m).html(k),$("#story .photo").attr("src",l).attr("alt",b).show()})}else// Display the "choose again" if there are no matches.
$("#story .photo").hide(),$("#story .text").load("/cctp623/stories/none.html")}))}),b.on("click",()=>{// Remove the attributes for the photo.
// Remove the text from the story.
$("#story .photo").attr("src","").attr("alt","").hide(),$("#story .text").html("")})});
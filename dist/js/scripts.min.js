...
let b,c,d,e,f,g=$("fieldset.gender input:checked").val(),h=$("fieldset.ethnicity input:checked").val();// Grab the user-selected attributes from the form.
null!=g&&null!=h&&(a.preventDefault(),$.getJSON("/cctp623/data/people.json",a=>{// Loop through each person.
$.each(a,(a,i)=>{g===i.gender&&h===i.ethnicity&&(b=i.name,c=i.pronounSubject,d=i.pronounObject,e=i.pronounPossAdj,f="/cctp623/photos/"+i.photo+".jpg")}),b?$("#story .text").load("/cctp623/stories/ss-intro-harry.html",()=>{// Change the selected words with the remixed text.

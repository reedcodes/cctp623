...
let b,c,d,e,f,g=$("fieldset.gender input:checked").val(),h=$("fieldset.ethnicity input:checked").val();// Declare some variables we need.
null!=g&&null!=h&&(a.preventDefault(),$.getJSON("/cctp623/data/people.json",a=>{// Loop through each person.
$.each(a,(a,i)=>{g===i.gender&&h===i.ethnicity&&(b=i.name,c=i.pronounSubject,d=i.pronounObject,e=i.pronounPossAdj,f="/cctp623/photos/"+i.photo+".jpg")}),b?($("#none").hide(),$("#story").show(),$("#story .name").html(b),$("#story .pronoun-subject").html(c),$("#story .pronoun-object").html(d),$("#story .pronoun-poss-adj").html(e),$("#story .photo").attr("src",f)):($("#none").show(),$("#story").hide())}))}),b.on("click",()=>{$("#none, #story").hide()})});
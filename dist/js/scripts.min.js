...
const a=$("button.remix"),b=$("button.restart");// Find the submit button to remix the page.
// When the "remix" button is clicked or entered...
// When the "restart" button is clicked or entered...
a.on("click",a=>{// Grab the user-selected attributes from the form.
let b=$("fieldset.gender input:checked").val(),c=$("fieldset.ethnicity input:checked").val();// Only remix the story if there are selected values...
if(null!=b&&null!=c){a.preventDefault();// Declare some variables we need.
let d,e,f,g,h;// Grab the defined attributes from the JSON.
$.getJSON("../../data/people.json",a=>{// Loop through each person.
// Display the remixed text.
$.each(a,(a,i)=>{b===i.gender&&c===i.ethnicity&&(d=i.name,e=i.pronounSubject,f=i.pronounObject,g=i.pronounPossAdj,h="photos/"+i.photo+".jpg")}),$("#story").addClass("remixed"),$("#story .name").html(d),$("#story .pronoun-subject").html(e),$("#story .pronoun-object").html(f),$("#story .pronoun-poss-adj").html(g),$("#story .photo").attr("src",h)})}}),b.on("click",()=>{$("#story").removeClass("remixed")})});
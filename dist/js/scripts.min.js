...
let b,c,d,e,f,g=$("fieldset.original input[ type=\"radio\" ]:checked").val(),h=$("fieldset.original input[ type=\"checkbox\" ]:checked").val(),j=$("fieldset.gender input[ type=\"radio\" ]:checked").val(),k=$("fieldset.ethnicity input[ type=\"radio\" ]:checked").val();// Grab the user-selected attributes from the form.
null!=g&&null!=j&&null!=k&&(a.preventDefault(),$.getJSON("/cctp623/data/people.json",a=>{// If we have a person...
if($.each(a,(a,g)=>{j===g.gender&&k===g.ethnicity&&(b=g.name,c=g.pronounSubject,d=g.pronounObject,e=g.pronounPossAdj,f="/cctp623/photos/"+g.photo+".jpg")}),b){// If "same story" is selected...
let a=h?h:g;// Load the correct story piece.
$("#story .text").load("/cctp623/stories/"+a+".html",()=>{$("#story ."+g).addClass("highlight"),$("#story .name."+g).html(b),$("#story .pronoun-subject."+g).html(c),$("#story .pronoun-object."+g).html(d),$("#story .pronoun-poss-adj."+g).html(e),$("#story .photo").attr("src",f).attr("alt",b).show()})}else// Display the "choose again" if there are no matches.

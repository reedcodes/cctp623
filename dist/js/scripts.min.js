...
a.on("click",a=>{// Declare some variables we need.
let b,c,d,e,f,g=$("fieldset.original input:checked").val(),h=$("fieldset.gender input:checked").val(),j=$("fieldset.ethnicity input:checked").val();// Grab the user-selected attributes from the form.
null!=g&&null!=h&&null!=j&&(a.preventDefault(),$.getJSON("/cctp623/data/people.json",a=>{// Loop through each person.
$.each(a,(a,g)=>{h===g.gender&&j===g.ethnicity&&(b=g.name,c=g.pronounSubject,d=g.pronounObject,e=g.pronounPossAdj,f="/cctp623/photos/"+g.photo+".jpg")}),b?$("#story .text").load("/cctp623/stories/ss-intro-"+g+".html",()=>{// Change the selected words with the remixed text.
// Insert the appropriate image for the character.
$("#story .name").html(b),$("#story .pronoun-subject").html(c),$("#story .pronoun-object").html(d),$("#story .pronoun-poss-adj").html(e),$("#story .photo").attr("src",f).attr("alt",b).show()}):($("#story .photo").hide(),$("#story .text").load("/cctp623/stories/none.html"))}))}),b.on("click",()=>{// Remove the attributes for the photo.
